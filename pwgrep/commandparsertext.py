...
IGNORE_CASE = u'Ignore case distinctions, so that characters that differ only' \
    u'in case match each other. Although this is straightforward when letters' \
    u'differ in case only via lowercase-uppercase pairs, the behavior is' \
    u'unspecified in other situations.  For example, uppercase “S” has an'\
    ...
    u'“ß” (U+00DF, LATIN SMALL LETTER SHARP S) is normally capitalized as the' \
    u'two-character string “SS” but it does not match “SS”, and it might not' \
    u'match the uppercase letter “ẞ” (U+1E9E, LATIN CAPITAL LETTER SHARP S) ' \
    u'even though lowercasing the latter yields the former. ‘-y’ is an ' \
...
ONLY_MATCHING = u'Print only the matched (non-empty) parts of matching lines, ' \
                u'with each such part on a separate output line.'
